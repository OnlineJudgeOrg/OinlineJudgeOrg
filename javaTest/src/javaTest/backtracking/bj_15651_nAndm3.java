...
public class bj_15651_nAndm3 {
	...
	public static void main(String[] args) throws IOException {
		...
		dfs(depth, m, n, availableNumber, resultNumber);
		System.out.println(sb.toString());
	}
	
	public static void dfs(int depth, int m, int n, int[] availableNumber, int[] resultNumber) {
		if (depth == m) {
			// 출력
		...
		for(int i=1; i<availableNumber.length; i++) {
			resultNumber[depth] = i;
			dfs(depth+1, m, n, availableNumber, resultNumber);
		}
	}
}
